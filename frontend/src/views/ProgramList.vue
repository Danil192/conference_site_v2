      ...
      <table class="table table-hover">
        ...
        <tbody>
          <tr v-for="item in items" :key="item.id">
            ...
            <td>{{ item.opisanie || '—' }}</td>
            <td>{{ formatDate(item.data_sozdaniya) }}</td>
            <td class="text-end">
      ...
      <div class="modal-dialog">
        <div class="modal-content">
          <div class="modal-header">
            <h5 class="modal-title">{{ isEdit ? 'Редактировать' : 'Добавить' }} контейнер программы</h5>
...
<script>
import { programAPI, konferentsiyaAPI } from '../services/api'
...
export default {
  ...
  methods: {
    ...
    async saveItem() {
      try {
        if (this.isEdit) {
          await programAPI.update(this.form.id, this.form)
        } else {
          await programAPI.create(this.form)
        }
        this.closeModal()
        this.fetchData()
    ...
    async deleteItem(id) {
      if (!confirm('Удалить контейнер программы?')) return
      try {
